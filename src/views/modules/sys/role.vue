<template>
  <div class="mod-role">
    ...
    <el-table
      ...
      <el-table-column prop="createTime" header-align="center" align="center" width="200" label="创建时间"></el-table-column>
...
<script>
  ...
  import request from '@/api/sys/role'
    ...
    methods: {
      ...
      deleteHandle (id) {
        ...
        _this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          request.deleteRoleByIds(ids).then(data => {
            if (data && data.code === 0) {
              _this.$message({
                message: '操作成功',
                type: 'success',
